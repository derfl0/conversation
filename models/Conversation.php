...
class Conversation extends SimpleORMap {
    ...
    public static function withUser($user) {
        $sql = "SELECT * FROM conversations a JOIN conversations b USING (conversation_id) WHERE a.user_id = ? and b.user_id = ? LIMIT 1";
    ...
    public static function createWithUser($user) {
        ...
        Conversation::create(array($new->id, $GLOBALS['user']->id, $other->getFullName()));
        Conversation::create(array($new->id, $other->id, $GLOBALS['user']->getFullName()));
        return $new;
    }
